...
class Solution {
    ...
    public static int Solve(String str,int[] need,int idx){
        if(idx==str.length()-1){
            return 0;
        }
        if(str.charAt(idx)==str.charAt(idx+1)){
            if(need[idx]>need[idx+1]){
                int temp = need[idx+1];
                need[idx+1] = need[idx];
                need[idx] =temp;
            }
               return Math.min(need[idx],need[idx+1])+Solve(str,need,idx+1);
        }else{
            return Solve(str,need,idx+1);
        }
    }
}
